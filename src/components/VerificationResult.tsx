...
import { Check, X, AlertTriangle, Info, ShieldCheck, Lock, KeyRound } from 'lucide-react';
...
interface Props {
  ...
  onDispense?: (credentialId: string, pickupPin?: string, notes?: string) => Promise<void>;
  credentialId?: string;
  dispensing?: boolean;
}

export default function VerificationResult({ result, onDispense, credentialId, dispensing = false }: Props) {
        ...
        {result.credential?.status === 'ACTIVE' && onDispense && credentialId && (
          isPass && (
            user && (user.role === 'PHARMACIST' || user.role === 'ADMIN') ? (
              <div className="mt-6 p-4 bg-white rounded-xl border border-purple-200 shadow-sm space-y-3.5 text-left">
                <div className="flex items-center gap-2 pb-2.5 border-b border-purple-100">
                  ...
                  <div>
                    <h4 className="text-xs font-bold uppercase tracking-wider text-purple-900">
                      Authorized Pharmacist Dispensation
                    </h4>
                ...
                {result.credential?.has_pickup_pin && (
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 mb-1 flex items-center justify-between">
                      <span className="flex items-center gap-1.5">
                        <KeyRound className="h-3.5 w-3.5 text-purple-600" />
                        Patient Pickup PIN <span className="text-red-500">*</span>
                      </span>
                      <span className="text-[10px] font-normal text-slate-400">Required 2FA</span>
                    </label>
                    <input
                      type="text"
                      value={pickupPin}
                      onChange={(e) => setPickupPin(e.target.value.trim())}
                      placeholder="Enter 6-digit PIN from patient..."
                      maxLength={12}
                      className="w-full px-3 py-2 text-sm font-mono tracking-wider border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 bg-slate-50/50"
                      required
                    />
                    <p className="text-[11px] text-slate-500 mt-1">
                      Ask the patient for the secret PIN shown in their digital wallet to prevent unauthorized claims.
                    </p>
                  </div>
                )}
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-700 mb-1">
                    Dispensation Notes (Optional)
                  </label>
                  <input
                    type="text"
                    value={dispenseNotes}
                    onChange={(e) => setDispenseNotes(e.target.value)}
                    placeholder="e.g. Brand substituted, dosage instructions explained..."
                    className="w-full px-3 py-1.5 text-xs border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
                  />
                </div>

                <button
                  onClick={() => onDispense(credentialId, pickupPin, dispenseNotes)}
                  disabled={dispensing || (!!result.credential?.has_pickup_pin && !pickupPin)}
                  className="w-full py-2.5 bg-purple-600 hover:bg-purple-700 disabled:opacity-50 text-white rounded-lg font-medium text-sm flex items-center justify-center gap-2 transition-colors cursor-pointer shadow-sm"
                >
                  <Check className="h-4 w-4" />
                  <span>{dispensing ? 'Recording Dispensation...' : 'Verify PIN & Dispense Medication'}</span>
                </button>
              </div>
            ) : (
              <div className="mt-6 p-4 bg-slate-900 text-slate-100 rounded-xl border border-slate-800 shadow-sm text-left">
                <div className="flex items-start gap-3">
                  <div className="p-2 rounded-lg bg-emerald-500/20 text-emerald-400 shrink-0 mt-0.5">
                    <ShieldCheck className="h-5 w-5" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-white text-sm">Dispensation Gated to Pharmacists</h4>
                    <p className="text-xs text-slate-300 mt-1 leading-relaxed">
                      This digital prescription is cryptographically verified and active. In accordance with safety policies, medication dispensation and refill deduction require an authenticated Pharmacist account.
                    </p>
                    {!user ? (
                      <Link
                        to="/login"
                        className="inline-flex items-center gap-1.5 mt-3 text-xs font-semibold bg-white text-slate-900 px-3 py-1.5 rounded-lg hover:bg-slate-100 transition-colors shadow-xs"
                      >
                        <Lock className="h-3.5 w-3.5" />
                        Pharmacist Sign In to Dispense
                      </Link>
                    ) : (
                      <div className="mt-2.5 text-xs text-amber-300 bg-amber-950/50 p-2 rounded border border-amber-800/40">
                        Signed in as <strong>{user.email}</strong> ({user.role}). Dispensation requires a licensed Pharmacist account.
                      </div>
                    )}
                  </div>
                </div>
